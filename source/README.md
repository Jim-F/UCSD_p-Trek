...
Every variable in the p-System is based on the 16-bit words used by the pseudo-machine. You can't get to individual bytes. An integer (in Fortran) is 16 bits, period (there is no "long int" INTEGER\*4). Characters are stored pair-wise in words. CHARACTER\*1 uses the first byte of a word, CHARACTER\*2 uses both bytes (but they cannot be compared directly, even if they contain the same character data), CHARACTER\*8 uses 4 words, etc. CHARACTER\*1 variables also cannot be compared directly with integers -- you have to use the intrinsic function ICHAR to convert the character to an integer (CHAR goes the other way). By the way, all intrinsic functions -- ABS, AMAX1, SIGN, SQRT, CHAR, ICHAR, etc. have to be explicitly declared as INTRINSIC.

The game source code sometimes spits out lines on the terminal whole (via subroutines "PROMPT" and "PROUT"). In these cases, the output buffer can be assigned an explicit character string such as " OLINE='THIS IS AN OUTPUT STRING' ". However, other output lines are built up in pieces (via the various "CRAM" subroutines). The barrier between packed (two-to-a-word) and unpacked (one-plus-ignored to a word) CHARACTER arrays in this dialect of Fortran, and the hoops I had to jump through to get around it, were a challenge.  And it is necessary to get around it, or it wouldn't be possible to control the spacing of things on the screen (or even parse input in the way the original code is set up to do). I resorted to temporary files (using FORMAT edits to control characters spacing) to get around that limitation. Numbers (such as the STARDATE) in the output line buffer are also formatted via temporary files.
...
The total memory available in the PCE emulator plus p-System as configured here is 128KB of RAM: 64KB allocated to stack plus heap, and 64KB allocated to the "external code pool". In original IBM PC hardware, this would have required 64KB on the motherboard, and a 64KB "Memory Expansion Option" card (from IBM; also called "Extended Memory" in the p-System documentation). The p-System's SETUP program ("SETUP.CODE" on the UTILITY distribution floppy) has been run to change SYSTEM.MISCINFO on the boot disk to use the extra 64KB for the external code pool.

The SETUP utility is discussed in the p-System "Users' Guide" (Book 2), beginning on p. 7-51 (p. 343 in the PDF). The settings relevant to "Extended Memory" are on:
...
salted throughout the code. Not all of them may be necessary, but **some** of them certainly are (and their removal will bring back the freezing at the point in the game where their subroutines are called).
